...
public class ExportExcel2 {
    ...
    public File export(String fileName) throws Exception {
        ...
        try {
         ...
         /*   HSSFSheet sheet = workbook.createSheet(title);*/
                HSSFSheet sheet =createWorkbook(workbook,g);
                // 单元格样式对象
                ...
                for (int i = 0; i < dataList.size(); i++) {
                    Object[] obj = dataList.get(tempnum);//遍历每个对象
     ...
     * 创建新工作表
     */
    public HSSFSheet createWorkbook(HSSFWorkbook workbook,int num) {

        HSSFSheet sheet = workbook.createSheet(title+num);
        logger.info("创建sheet",num);
